				...
				<table>
					<tr>
						...
						<td><form:input path="numStudent" name="numStudent"
								disabled="true" /> <form:errors path="numStudent"
								cssClass="error" /></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><input name="oldPassword" type="password" /> <c:if
								test="${!empty errorOldPassword}">
								<span class="error"><c:out value="${errorOldPassword}" /></span>
							</c:if></td>
					</tr>
					<tr>
						<td><label>Nouveau mot de passe (laisser vide si non
								souhaité)</label></td>
						<td><input name="newPassword" type="password" value="" /></td>
					</tr>
					<tr>
						<td><label>Confirmation nouveau mot de passe (laisser
								vide si non souhaité)</label></td>
						<td><input name="confirmation" type="password" value="" /> <c:if
								test="${!empty errorConfirmation}">
								<span class="error"><c:out value="${errorConfirmation}" /></span>
							</c:if></td>
					</tr>
					<tr>
						<td><form:label path="adress">Adresse</form:label></td>
						<td><form:input path="adress" type="text" /> <form:errors
								path="adress" cssClass="error" /></td>
					</tr>

					<tr>
						<td><form:label path="promo">Promotion</form:label></td>
						<td><form:input path="promo" type="text" /> <form:errors
								path="promo" cssClass="error" /></td>
					</tr>
					...
					<tr>
						...
						<td><select id="tokenizer" class="tokenizer" multiple>
								<c:if test="${!empty hobbies}">
									<c:forEach items="${hobbies}" var="hobby">
										<c:choose>
											<c:when
												test="${fn:contains(sessionScope.userSession.hobbies, hobby)}">
						...
						</select> <c:set var="hobbies" value="" /> <c:forEach
								items="${sessionScope.userSession.hobbies}" var="hobby">
								<c:set var="hobbies" value="${hobby.nameH},${hobbies}" />
							</c:forEach> <input name="hobbies" id="selectedValues" type="hidden"
							value="${hobbies}" /> <c:if test="${!empty errorHobbies}">
					...
					<tr>
						<td></td>
						<td><input id="informationSubmit" class="submit"
							type="submit" value="Valider">
						<a
							href="${pageContext.request.contextPath}/workspace/index.htm">
							<input type="button" value="Annuler" class="submit" /></a></td>
					</tr>
				</table>
			</form:form>
		</div>
